...
class BBB_Traj(object):
    ...
    def load_data(self):
        ...
        print('Separate {} data based on deviation threshold {}'.format(fp.shape[0], self.maximum_separation_distance))
    ...
    def build_graph(self, keep_prob=None):
        ...
        with tf.variable_scope("RCBBB", reuse=tf.AUTO_REUSE):
            ...
            conv1 = tfp.layers.Convolution3DFlipout(filters=3,
                                                   kernel_size=[1, 5, 5],
                                                   strides=[1, 3, 3],
                                                   padding='valid',
                                                   activation=activa,
            ...
            conv2 = tfp.layers.Convolution3DFlipout(filters=1,
                                                   kernel_size=[1, 3, 3],
                                                   strides=[1, 3, 3],
                                                   padding='same',
                                                   activation=activa,
            ...
            dense1 = tfp.layers.DenseFlipout(4, activation=activa, name='dense1')(flat1)
            dense2 = tfp.layers.DenseFlipout(1, activation=activa, name='dense2')(dense1)
            ...
            lstm_cell = ed.layers.LSTMCellFlipout(self.lstm_hidden_dim, activation=activa_lstm)
            ...
            dense3 = tfp.layers.DenseFlipout(int(self.lstm_hidden_dim/2),
                                            activation=activa,
                                            name='dense3')(lstm_out)

            dense4 = tfp.layers.DenseFlipout(int(self.lstm_hidden_dim/4),
                                            activation=activa,
                                            name='dense4')(dense3)

            dense5 = tfp.layers.DenseFlipout(3, activation=activa, name='dense5')(dense4)
            ...
            # self.nll = tf.losses.huber_loss(labels=self.traj[:, 1:, :], predictions=self.outputs, weights=1.0, delta=0.2)

            self.nll = tf.sqrt(tf.reduce_mean((self.traj[:, 1:, :] - self.outputs)**2))

            # self.nll = tf.sqrt(tf.reduce_mean(tf.square(self.outputs[:, :, 0] - self.traj[:, 1:, 0]) +
            #                                   tf.square(self.outputs[:, :, 1] - self.traj[:, 1:, 1]) +
            #                                   tf.square(self.outputs[:, :, 2] - self.traj[:, 1:, 2])))

            self.kl = sum(model.losses)/self.fp_train.shape[0]/100

            self.loss = self.kl + self.nll

    def build_graph_nornn(self, keep_prob=None):

        with tf.variable_scope("RCBBB", reuse=tf.AUTO_REUSE):

            self.cube = tf.keras.Input(shape=(49, 32, 32, 1))
            self.fp = tf.keras.Input(shape=(50, 2))

            conv1 = tfp.layers.Convolution3DReparameterization(filters=3, kernel_size=[1, 5, 5], strides=[1, 3, 3], padding='valid',
                                     activation=tf.nn.tanh, name='conv1')(self.cube)

            conv2 = tfp.layers.Convolution3DReparameterization(filters=1, kernel_size=[1, 3, 3], strides=[1, 3, 3], padding='same',
                                     activation=tf.nn.tanh, name='conv2')(conv1)

            flat1 = tf.keras.layers.Reshape([self.input_dimension - 1, 4 * 4 * 1], name='flat1')(conv2)
            ...
            # recurrent cell need to be changed
            lstm_out = tf.keras.layers.concatenate([self.fp[:, 1:, :], dense2], axis=-1)

            dense3 = tfp.layers.DenseReparameterization(int(self.lstm_hidden_dim/2), activation=tf.nn.tanh, name='dense3')(lstm_out)
            dense4 = tfp.layers.DenseReparameterization(int(self.lstm_hidden_dim/4), activation=tf.nn.tanh, name='dense4')(dense3)
            dense5 = tfp.layers.DenseReparameterization(2, activation=tf.nn.tanh, name='dense5')(dense4)
            ...
            self.nll = tf.reduce_mean(tf.sqrt(tf.square(self.outputs[:, :, 0] - self.traj[:, 1:, 0]) +
                                 tf.square(self.outputs[:, :, 1] - self.traj[:, 1:, 1])), axis=None)
    ...
    def train_model(self):
        ...
        learning_rate_kl = td.poly_inverse_time_decay(0.001, global_step, decay_steps=1, decay_rate=0.0001, power=0.75)
        learning_rate_nll = td.poly_inverse_time_decay(0.001, global_step, decay_steps=1, decay_rate=0.0001, power=0.75)

        # optimizer choice
        #train_op = tf.train.MomentumOptimizer(learning_rate, momentum=0.9, use_nesterov=True).minimize(self.loss, var_list=vars)
        train_op = tf.train.AdamOptimizer(learning_rate_nll).minimize(self.loss, var_list=vars)
        #train_op = tf.train.AdagradOptimizer(learning_rate, initial_accumulator_value=0.1).minimize(self.loss, var_list=vars)
        #train_op = tf.train.RMSPropOptimizer(learning_rate, decay=0.9, momentum=0.2, epsilon=1e-10, use_locking=False).minimize(self.loss, var_list=vars)
        train_op_nll = tf.train.AdamOptimizer(learning_rate_nll).minimize(self.nll, var_list=vars)
        train_op_kl = tf.train.AdamOptimizer(learning_rate_kl).minimize(self.kl, var_list=vars)
        ...
        for j in range(self.epoch):
            for i in range(batch_num):
                ...
                _, loss, kl_train, nll_train = self.sess.run([train_op_nll, self.loss, self.kl, self.nll], feed_dict={self.fp: train_fp_batch,
                                                                                                                  self.traj: train_traj_batch,
                                                                                                                  self.cube: train_cube_batch})
    ...
    def test_model(self, draw_figure=False, pre_trained=False):
        ...
        for test_num in range(self.test_number):
            ...
            #test_pred_traj = self.inverse_normalization(pred_traj)
        ...
        np.save('flipout_{}_l2.npy'.format(self.sector), self.l2_new)
...
if __name__ == '__main__':
    ...
    cfg = {'date': 20190624,
           ...
           'test_number': 100, }
    cfg['save_path'] = './Flipout_{}_{}_epoch_{}'.format(cfg['date'], cfg['sector'], cfg['epoch'])
