...
class BBB_Traj(object):
    ...
    def load_data(self):
        ...
        # train test split
        self.dev_train, self.dev_test = train_test_split(dev, test_size=self.test_ratio, shuffle=False, random_state=None)
    ...
    def build_graph(self, keep_prob=None):
        ...
        with tf.variable_scope("RCBBB", reuse=tf.AUTO_REUSE):
            ...
            dense1 = tfp.layers.DenseFlipout(4, activation=tf.nn.tanh, name='dense1')(flat1)
            dense2 = tfp.layers.DenseFlipout(1, activation=tf.nn.tanh, name='dense2')(dense1)
            ...
            dense5 = tfp.layers.DenseFlipout(2, activation=tf.nn.tanh, name='dense8')(dense4)

            self.outputs = dense5

            model = tf.keras.Model(inputs=[self.cube, self.fp], outputs=self.outputs)

            print(model.summary())

            # mae1 = tf.abs(self.outputs[:, :, 0] - self.traj[:, 1:, 0])
            # mae2 = tf.abs(self.outputs[:, :, 1] - self.traj[:, 1:, 1])
            # self.nll = tf.reduce_mean(mae1 + mae2)

            # self.nll = tf.reduce_mean(tf.sqrt(tf.square(self.outputs[:, :, 0] - self.traj[:, 1:, 0]) +
            #                                   tf.square(self.outputs[:, :, 1] - self.traj[:, 1:, 1])), axis=None)
            self.nll = tf.losses.huber_loss(labels=self.dev[:, 1:, :], predictions=self.outputs, weights=1.0, delta=1.0)

            # self.nll = tf.reduce_mean(tf.sqrt(tf.square(self.outputs[:, :, 0] - self.dev[:, 1:, 0]) +
            #                                   tf.square(self.outputs[:, :, 1] - self.dev[:, 1:, 1])), axis=None)
    ...
    def train_model(self):
        ...
        # define variables
        self.fp = tf.placeholder(tf.float32, [None, self.input_dimension, 2], name='fp')
        self.cube = tf.placeholder(tf.float32, [None, self.input_dimension-1, 32, 32, 1], name='cube')
        self.dev = tf.placeholder(tf.float32, [None, self.input_dimension, 2], name='dev')
        ...
        learning_rate = td.poly_inverse_time_decay(0.01, global_step, decay_steps=1, decay_rate=0.0001, power=0.75)
        ...
        for j in range(self.epoch):
            for i in range(batch_num):
                ...
                train_dev_batch = self.dev_train[self.batch_size * i:self.batch_size * (i + 1), :, :]
                ...
                loss_test, kl_test, nll_test = self.sess.run([self.loss, self.kl, self.nll], feed_dict={self.fp: self.fp_test[:self.batch_size, :, :],
                                                                self.dev: self.dev_test[:self.batch_size, :, :],
    ...
    def test_model(self, draw_figure=False, pre_trained=False):
        ...
        for test_num in range(self.test_number):
            ...
            for i in range(self.batch_num):
                ...
                test_dev_batch = self.dev_test[self.batch_size * i:self.batch_size * (i + 1), :, :]
                test_cube_batch = self.cube_test[self.batch_size * i:self.batch_size * (i + 1), :, :, :, :]

                pred_dev_batch, test_loss = self.sess.run([self.outputs, self.loss], feed_dict={self.fp: test_fp_batch,
                                                                                            self.dev: test_dev_batch,
                                                                                            self.cube: test_cube_batch})
                if i==0:
                    pred_dev = pred_dev_batch
                else:
                    pred_dev = tf.concat([pred_dev, pred_dev_batch], axis=0, name='stack_test')

                print('Batch: {} Testing Loss: {}'.format(i, test_loss))

            # do inverse normalization to the prediction
            # fp + dev = pred_traj
            test_pred_traj = pred_dev.eval(session=self.sess) + self.fp_test[:self.batch_size*self.batch_num, 1:, :]
            np.save('pred_dev.npy', pred_dev.eval(session=self.sess))
        ...
        np.save('2dflipout_{}_l2.npy'.format(self.sector), self.l2_new)
        np.save('2dtestset_{}_l2.npy'.format(self.sector), self.l2_ori)
...
if __name__ == '__main__':
    ...
    cfg['save_path'] = './Flipout2d_{}_{}_epoch_{}'.format(cfg['date'], cfg['sector'], cfg['epoch'])
