...
class BBB_Traj(object):
    ...
    def build_graph_dropout(self, keep_prob):
        ...
        with tf.variable_scope("RCNN", reuse=tf.AUTO_REUSE):
            ...
            drop3 = tf.nn.dropout(dense1, keep_prob)

            dense2 = tf.layers.dense(drop3, 1, activation=tf.nn.tanh, name='dense2')
            #drop4 = tf.nn.dropout(dense2, keep_prob)
            ...
            lstm_cell = tf.nn.rnn_cell.DropoutWrapper(lstm_cell,
                                                      ...
                                                      dtype=tf.float32,
                                                      input_size=lstm_in.get_shape()[-1], )
            # init_state = tf.contrib.rnn.LSTMStateTuple(self.fp[:, 0, :], self.fp[:, 0, :])
            ...
            drop5 = tf.nn.dropout(dense3, keep_prob)

            dense4 = tf.layers.dense(drop5, self.lstm_hidden_dim / 4, activation=tf.nn.tanh, name='dense4')
            drop6 = tf.nn.dropout(dense4, keep_prob)

            dense5 = tf.layers.dense(drop6, 2, activation=tf.nn.tanh, name='dense5')
            self.outputs = dense5
    ...
    def train_model(self):
        ...
        print('build graph..........')
        self.build_graph_dropout(self.keep_prob)
    ...
    def test_model(self, draw_figure=False, pre_trained=False):
        ...
        np.save('2ddrop_{}_l2.npy'.format(self.sector), self.l2_new)
...
if __name__ == '__main__':
    ...
    cfg['save_path'] = './2dDrop_{}_{}_epoch_{}'.format(cfg['date'], cfg['sector'], cfg['epoch'])
