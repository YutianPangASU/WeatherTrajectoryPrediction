...
class cGAN_Traj(object):
    ...
    def load_data(self):
        ...
        z = self.sample_from_distribution(dim=[fp.shape[0], self.input_dimension, 2])
    ...
    def sample_from_distribution(self, dim):
        ...
        lower, upper = 0, 1
        mu, sigma = 0.5, 1.0
        #z = stats.truncnorm.rvs((lower - mu) / sigma, (upper - mu) / sigma, loc=mu, scale=sigma, size=dim)
        z = np.random.uniform(lower, upper, size=dim)
        return z
    ...
    def build_graph(self):
        def generator(cube, fp, gc, reuse=False):
            with tf.variable_scope("GAN/Generator", reuse=tf.AUTO_REUSE):
                ...
                drop4 = tf.nn.dropout(features, 0.5)
                ...
                lstm_in = tf.concat([gc[:, 1:, :], fp[:, 1:, :], drop4], -1)

                # multi layers lstm
                # rnn_layers = [tf.nn.rnn_cell.LSTMCell(size) for size in [2]]
                # multi_rnn_cell = tf.nn.rnn_cell.MultiRNNCell(rnn_layers)
                # outputs, _ = tf.nn.dynamic_rnn(cell=multi_rnn_cell, inputs=lstm_in, dtype=tf.float32)

                # # Seq2Seq learning with lstm
                lstm_cell = tf.nn.rnn_cell.LSTMCell(self.lstm_hidden_dim)
                # lstm_cell_2 = tf.nn.rnn_cell.LSTMCell(2)

                # # define initial states of lstm
                init_state = tf.contrib.rnn.LSTMStateTuple(fp[:, 0, :], fp[:, 0, :])
                lstm_outputs, states = tf.nn.dynamic_rnn(cell=lstm_cell, inputs=lstm_in, initial_state=init_state)
                # lstm_outputs2, _ = tf.nn.dynamic_rnn(cell=lstm_cell_2, inputs=lstm_outputs1, initial_state=states)
        ...
        def discriminator(x, fp, cube, reuse=False):
            with tf.variable_scope("GAN/Discriminator", reuse=tf.AUTO_REUSE):
                ...
                drop2 = tf.nn.dropout(dense3, 0.5)
                features = tf.layers.dense(drop2, 1, activation=None, name='dense4')
                drop3 = tf.nn.dropout(features, 0.5)

                # concatenate gc(noise) [Nx50x2], fp(condition) [Nx50x2] and weather features extracted [Nx49x1]
                # The final input for lstm cell should be dimension [Nx49x5]
                lstm_in = tf.concat([x[:, :, :], fp[:, 1:, :], drop3], -1)
        ...
        G_generate = generator(self.cube, self.fp, self.gc)  # generate data from output using noise gc(physics based noise)
        self.D_generate = discriminator(G_generate, self.fp, self.cube, reuse=True)  # fake from generator then feed into discriminator
        ...
        D_real = discriminator(self.traj[:, 1:, :], self.fp, self.cube)
    ...
    def train_model(self):
        ...
        # optimizer
        # gen_step = tf.train.RMSPropOptimizer(learning_rate=0.0001).minimize(self.fake_loss, var_list=gen_vars)  # G Train step
        # disc_step = tf.train.RMSPropOptimizer(learning_rate=0.0001).minimize(self.real_loss, var_list=disc_vars)  # D Train step
        # gen_step = tf.train.GradientDescentOptimizer(learning_rate=0.001).minimize(self.fake_loss, var_list=gen_vars)
        # disc_step = tf.train.GradientDescentOptimizer(learning_rate=0.001).minimize(self.real_loss, var_list=disc_vars)
        gen_step = tf.train.AdamOptimizer(learning_rate=0.001).minimize(self.fake_loss, var_list=gen_vars)
        disc_step = tf.train.AdamOptimizer(learning_rate=0.001).minimize(self.real_loss, var_list=disc_vars)
        ...
        for j in range(self.epoch):
            for i in range(batch_num):
                ...
                for _ in range(self.generator_steps):
                    _, dloss = sess.run([disc_step, self.real_loss], feed_dict={self.fp: train_fp_batch,
                ...
                for _ in range(self.discriminator_steps):
                    _, gloss = sess.run([gen_step, self.fake_loss], feed_dict={self.fp: train_fp_batch,
                                                                               self.gc: train_gc_batch,
                                                                               self.traj: train_traj_batch,
                                                                               self.cube: train_cube_batch})
    ...
    def test_model(self, draw_figure=False):
        # define variables
        self.fp = tf.placeholder(tf.float32, [None, self.input_dimension, 2])
        self.gc = tf.placeholder(tf.float32, [None, self.input_dimension, 2])
        self.cube = tf.placeholder(tf.float32, [None, self.input_dimension - 1, 32, 32, 1])
        ...
        for test_num in range(self.test_number):
            ...
            for i in range(self.batch_num):
                ...
                test_gc_batch = self.sample_from_distribution(dim=[self.batch_size, self.input_dimension, 2])
                ...
                pred_traj_batch, test_loss = sess.run([self.D_generate, self.fake_loss], feed_dict={self.fp: test_fp_batch,
    ...
    def draw_figure(self):
        ...
        for idx in range(self.figure_number):
            ...
            plt.ylabel('Latitude')
            plt.plot(self.pred_traj_mean[idx, 1:, 1], self.pred_traj_mean[idx, 1:, 0], '*r', markersize=2, label='predicted_track')

            # draw confidence ellipse
            ells = [Ellipse(xy=[self.pred_traj_mean[idx, i, 1], self.pred_traj_mean[idx, i, 0]],
                            width=30 * self.pred_traj_std_lon[idx, i],
                            height=30 * self.pred_traj_std_lat[idx, i],
                            #angle=0)
                            angle=90-np.rad2deg(
                    ...
                    for i in range(1, self.input_dimension-1)]
...
if __name__ == '__main__':
    ...
    fun = cGAN_Traj(cfg)
    #fun.train_model()
    fun.test_model(draw_figure=True)
